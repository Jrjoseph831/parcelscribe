      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-10">
        ...
        <section className="grid gap-4 md:grid-cols-2">
          {guides.map((guide) => (
            <Link
              ...
              className="flex h-full flex-col gap-2 rounded-xl border border-gray-200 bg-white px-4 py-4 shadow-sm transition hover:border-blue-200"
            >
              <p className="text-sm font-semibold text-gray-900">{guide.title}</p>
              <p className="text-sm text-gray-700 sm:text-base">{guide.description}</p>
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr,260px]">
          <div className="space-y-10">
            {guides.map((guide) => (
              <section key={guide.slug} id={guide.slug} className="scroll-mt-24 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm sm:p-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <Link className={`${buttonClasses("secondary")} w-full sm:w-auto`} href="/builder">Start packet</Link>
                </div>
                <div className="mt-4 space-y-3 text-sm text-gray-800 sm:text-base">
                  {guide.body.split("\n\n").map((para) => (
                    <p key={para}>{para}</p>
                  ))}
                </div>
